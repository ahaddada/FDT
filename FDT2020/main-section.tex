...
Let $x$ be a Carr-Vempala point defined on a graph $G=(V,E)$.  From the above discussion, we can write $\frac{|V|-1}{|V|}\cdot x= \sum_{i=1}^{k}\lambda_i\chi^{T_i}$ where $T_i$ is spanning tree of $G$, $\chi^{T_i}$ is its incidence vector, $\sum_{i=1}^{k}\lambda_i=1$, and $\lambda_i\geq 0$ for $i\in [k]$. The spanning-tree polytope is integral so there are polynomial-time algorithms to construct this decomposition (See Proposition~\ref{cara} and surrounding discussion).

Let $O_i$ be the set of odd-degree vertices of spanning tree $T_i$. Now consider the incidence vectors of feasible $O_i$-joins. Consider any subset of vertices $S$ with an even number of odd-degree vertices.  Then the $O_i$-join LP requires at least one edge joining $S$ to the rest of the vertices. The subtour-elimination LP requires every subset of vertices have two edges connecting it to the rest of the graph.  Therefore, since $x$ is a feasible solution to the subtour-elimination LP, then $x/2$ is feasible for the $O_i$-join associated with spanning tree $T_i$.
Thus we have $\frac{x}{2}$ is in the convex hull of incidence vectors of $O_i$-join of $G$, denoted $O_i\join(G)$.
