...
We also make the following assumption that make the presentation of the results simpler: for $\ell \in [n]$ the set $\{x\in P: x_\ell = 0\}$ is empty, we assume that every point in $P$ has 1 in the $\ell$-th coordinate, i.e. constraints matrix $A$ contains the valid constraint that $x_\ell \geq 1$. We make this assumption to enusre that we always have a two branches when branching on a fractional coordinate of a point.
