...
\cindy{TODO: Is this something that should be clear to the reader with what they know at this point? Does it need an explanation? Does it need an explanation at the end of this section or the next?}
...
For all 450 instances in our experiments, running FDT on the LP optimal (fractional extreme point) of the cut LP gave a feasible solution with value at most a factor $\frac{3}{2}$ larger than the LP lower bound.  Such a feasible solution gives an upper bound on the integrality gap $g(\tap)$ of that specific instance of at most $\frac{3}{2}$.
...
For 425 instances, the integrality gap upper bound was $\frac{4}{3}$, for 22 instances it was $\frac{6}{5}$, for 2 instances it was $\frac{8}{7}$.
...
Hence, we can write $\frac{|V|-1}{|V|}\cdot x= \sum_{i=1}^{k}\lambda_i\chi^{T_i}$ where $T_i$ is spanning tree of $G$, $\chi^{T_i}$ is its incidence vector, $\sum_{i=1}^{k}\lambda_i=1$, and $\lambda_i\geq 0$ for $i\in [k]$. Let $O_i$ be the set of odd-degree vertices of $T_i$. It is easy to see that $\frac{x}{2}$ is in the convex hull of incidence vectors of $O_i$-join of $G$, namely $O_i\join(G)$~\footnote{For graph $G=(V,E)$ and $O\subseteq V$ with $|O|$ even, an $O$-join of $G$ is a subgraph of $G$ that has odd degree on the vertices in $O$ and even degree on vertices in $V\setminus O$.}.
\cindy{Here is another instance of ``It's easy to see that...'' Given that (quite reasonably) you do not expect the reader to know what an O-join is, you should probably just give the short justification for this statement and remove ``It's easy to see that.'' Also, do you mean that $x/2$ is in the convex hull of the incident vectors of all $O_i$-joins for any spanning tree $T_i$?  If so, it would be good to make that clear.  Since I will not make it through every word of Sections 2-4 if you submit today, I recommend looking for similar places in those sections.}


\cindy{Do you have to precompute the joins?  Does that mean generating spanning trees before setting up the LP?  Or generating them on the fly?  Only minimum-weight spanning trees?  I haven't had a lot of time to think about this.}
