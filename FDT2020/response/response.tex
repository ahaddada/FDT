...
\response{We thank the reviewers for their insightful and constructive comments. We sincerely apologize for the long delay in submitting our revision.  We thank our editor very much for the extension. Our junior author finished his training and took a job in industry. The more senior co-author at a national lab accepts her considerable share of responsibility for the delay as well.
}
...
\response{We have addressed the issue in the proof of Theorem 3 as detailed below. We also made several major revision in our presentation especially with regards to the more technical parts of the paper.}
\subsubsection{Minor Comments}
\begin{itemize}
\item Introduction: Introduction jumps between describing methods and problems the methods are applied to. There is a subsection 1.2 Experiments with more problem descriptions. Please restructure the introduction so
...
\response{We made various changes starting with the paragraph before Definition 1 through the discussion that follows. We compare our Definition 1 to other definitions of the integrality gap. Our definition is the integrality gap of a polyhedron. We explain why we use this definition. We also added more context about our definition of integrality gap to the abstract, so the reader is aware immediately that we are using a polyhedron-based definition.}
...
\response{As stated above, the discussion following Definition 1. We added a remark that $c$ must be non-negative for Carr-Vempala to work.} 
...
\response{We respect and understand this reviewer's point of view. We have added some caveats to the start of the experimental section. Our experiments are intended to demonstrate using FDT for experimentally studying integrality gaps. We have explicitly stated the we are not recommending FDT as a replacement heuristic. We hope it will complement other general heuristics. %Our comparison of FDT to feasibility pump is mainly to showcase a use case of the algorithm and a direction that could be possibly explored. We are not suggesting FDT as a replacement for FP. We will add remarks that make this clear to the reader.

With respect to Christofides, we mistakenly wrote that we compared FDT to Christofides.  In fact, we compare FDT with BOMC (Best of Many Christofides) - this has been fixed in the abstract and the paper. As presented in Figure 3, we don't settle for the worst-case upper bound of 3/2. Similarly, for TAP, FJ81 is the only rounding algorithm known for the solution of the CUT-LP. Again, we don't use the worst-case upper bound of 2. Instead we use the instance-specific gap provided by the algorithm. Also, FDT is a generic algorithm that applies to any formulation with a suitablly bounded integrality gap, while BOMC and FJ81 problem-specific.}
...
\response{We made several edits to the abstract. We believe they address all the points mentioned above.}
...
\response{We felt that readers might be curious about how FDT compares to another general incumbent heuristic. We were not able to find any code for feasibility pump 2.0. In the FP 2.0 paper, the authors wrote that the “overhead of constraint propagation can be quite large … Hence we need a very efficient implementation of the whole constraint propagation system, including both the single propagators and the overall propagation engine.”  FP 2.0 is much more complex than simple FP. To be fair to FP 2.0 in a comparison, we would need to program the authors’ optimizations to achieve their performance.  Since our goal was to show that the performance can be reasonable, we felt that implementing the full, optimized FP 2.0 was out of scope.}
%The comparisons with FP are not presented to claim that FDT to provide direct comparisons between the two algorithms, but to provide a baseline on the IG upper bounds provided by another LP-based algorithm. We want to emphasize that the scope of this work in mostly on the theoretical contributions, rather than experimental results.
...
\response{The proof of this theorem only holds for the dominant of $S(I)$ and not $S(I)$ itself, although for many problems of interest we have $S(I)=\mathcal{D}(S(I))$. }
...
\response{
DomToIP starts from a feasible solution to the LP relaxation and tries to find an integer feasible solution in the support of the fractional solution. If Dom2IP fails to do this for any instance, we can conclude the integrality gap is infinite. We could perform a broad search over possible (useful/interesting) feasible points to the LP relaxation to find such a certificate for the unboundedness of the integrality gap.}
