...
There are fairly good bounds for the integrality gap for the Traveling Salesman Problem (TSP) or 2EC (see \cite{tspbook})
...
\arash{I added this figure \ref{fig:CVpoint} but maybe we can put it later in the experiments section to make the introduction shorter?}
...
\end{tikzpicture}
\caption{A Carr-Vempala point with 8 vertices on its cycle. Dashed edges represent paths where each edge on the path have $x_e =1$.}
\label{fig:CVpoint}
\end{figure}
