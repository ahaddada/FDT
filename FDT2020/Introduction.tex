...
\begin{itemize}
\item We give an algorithm to express any feasible point for the LP relaxation of a binary IP (or a \{0,1,2\} IP) as a convex combination of feasible solutions, provided the IP integrality gap is bounded.  This is the Fractional Decomposition Tree (FDT) algorithm.
\item We give a simpler, faster algorithm algorithm to give one feasible solution for any IP with bounded integrality gap. If this algorithm fails, then the integrality gap is infinite.
