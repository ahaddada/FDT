...
In Section~\ref{sec:binaryfdt} we introduce the \textit{Fractional Decomposition Tree Algorithm} (FDT) which is polynomial time algorithm. Given a point $x\in P(I)$ FDT produces a convex combination of feasible points in $S(I)$ that are dominated coordinatewise by a ``factor" $C$ times $x$.
