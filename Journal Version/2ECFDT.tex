...
\FDTEC*
We do not know the exact value for $g_{\EC}$, but we know $\frac{6}{5} \leq g_{\EC} \leq \frac{3}{2}$ \cite{carr-ravi,Wolsey1980}. Also, we need to remark that the polyhedral analysis of Christofides' algorithm provides a $\frac{3}{2}$-approximation for 2EC, i.e. we already have an algorithm with $C\leq \frac{3}{2}$ \cite{Wolsey1980}. However, we will show in Section \ref{experiment} that in practice the constant $C$ for the FDT algorithm for 2EC is much better than $\frac{3}{2}$ for FEPs of order 10.
...
\begin{restatable}{lemma}{2ECLPC}
	\label{LPC2EC}
	Given $x\in \EC(G)$, and $e\in E$ we can find in polynomial time vectors $\hat{x}^0,\hat{x}^1$ and $\hat{x}^2$ and scalars $\gamma_0,\gamma_1$, and $\gamma_2$ such that: (i) $\gamma_0 + \gamma_1 +\gamma_2 \geq \frac{ 1}{g_{\EC}}$, (ii) $\hat{x}^0,\hat{x}^1,$ and $\hat{x}^2$ are in  $ \EC(G)$, (iii) $\hat{x}^0_e=0$, $\hat{x}^1_e=1$, and $\hat{x}^2_e=2$, (iv) $\gamma_0 \hat{x}^0 + \gamma_1{\hat{x}}^1  + \gamma_2\hat{x}^2\leq {x}$, (v) for $f\in E$ with ${x}_f\geq 1$, we have $\hat{x}^j_f\geq 1$ for $j=0,1,2$.
\end{restatable}
